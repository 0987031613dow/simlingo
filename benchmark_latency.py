...
def run_benchmark(model, device: torch.device, num_runs: int, warmup: int):
    ...
    for i in range(num_runs):
        ...
        t_language.append(getattr(model, "_timing_language_ms", None))
        t_waypoint.append(getattr(model, "_timing_waypoint_ms", None))

        print(f"  run {i+1:3d}/{num_runs} │ total={total_ms:7.1f} ms"
              f" │ vision={t_vision[-1] or '?':>7}ms"
              f" │ language={t_language[-1] or '?':>7}ms"
              f" │ waypoint={t_waypoint[-1] or '?':>7}ms")

    return t_total, t_vision, t_language, t_waypoint
...
def main():
    ...
    t_total, t_vision, t_language, t_waypoint = run_benchmark(
        model, device, args.runs, args.warmup
    )

    # 計算「其他」時間（preprocessing / adaptor 等）
    t_other = []
    for tot, vis, lang, wp in zip(t_total, t_vision, t_language, t_waypoint):
        if all(v is not None for v in [vis, lang, wp]):
            t_other.append(tot - vis - lang - wp)
    ...
    summarize("Language Generation",      t_language)
    summarize("Waypoint Generation",      t_waypoint)
    summarize("Other (adaptor/overhead)", t_other)
    print("=" * 75)
